...
def lift_segment(ne: NEHeader, seg_num: int, func_offset: int = -1, xmod=None):
    ...
    print(f'/* Auto-generated by ne_lift.py - Dance eJay recomp */')
    print()
    ...
    for func in target_funcs:
        ...
        def _unreachable(m):
            ...
            return (f'ejay_unreachable("{seg.index:03d}", 0x{addr - seg.file_offset:04X});'
        ...
        # Any same-segment callee we did not lift becomes ejay_unreachable().
        def _unknown_label(m):
            if m.group(1) in valid_labels:
                return m.group(0)
            return (f'ejay_unreachable("{seg.index:03d}", 0x{m.group(2)});'
