...
maps) and are written to work/win16_imports.json by tools/ida_export.py.
...
_ANALYSIS = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'work'))
...
# ---------------------------------------------------------------------------
# DANCE02.DLL's imports. Win16 is PASCAL: the callee pops its arguments, so a
# missing or wrong purge here does not fail at the call - it shifts the
# CALLER's frame, and the damage surfaces in its epilogue restoring DS from the
# wrong slot. Sizes: WORD/UINT/HANDLE/BOOL/int/BYTE = 2; DWORD/LONG/far ptr = 4.
#
# The audio engine's whole surface is here: waveOut/waveIn for playback and
# recording, aux* for the master volume, timeSetEvent for the sequencer tick.
PURGE.update({
    # ---- KERNEL ----
