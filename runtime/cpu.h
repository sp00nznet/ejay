 ...
 * call. Opt into stderr-per-call with -DEJAY_TRACE_FN. */
#define EJAY_FN_RING_BITS 12
#define EJAY_FN_RING_SIZE (1u << EJAY_FN_RING_BITS)
extern const char *g_fn_ring[EJAY_FN_RING_SIZE];
...
void ejay_sp_check(const char *nm);
#define TRACE_FN(n) do { g_fn_ring[(g_fn_ring_pos++) & (EJAY_FN_RING_SIZE-1)] = (n); \
    ejay_sp_check(n); } while (0)
#elif defined(EJAY_TRACE_FN)
#define TRACE_FN(n) do { g_fn_ring[(g_fn_ring_pos++) & (EJAY_FN_RING_SIZE-1)] = (n); \
    fprintf(stderr, "FN %s\n", (n)); } while (0)
#else
...
#define TRACE_FN(n) do { g_fn_ring[(g_fn_ring_pos++) & (EJAY_FN_RING_SIZE-1)] = (n); if (g_fncount | g_watch_ds) ejay_fn_hit(n); } while (0)
#endif
...
void ejay_unreachable(const char *seg, unsigned off);
...
void ejay_bp_broke(const char *callee, uint16_t bp0, uint16_t bp1, uint16_t sp0, uint16_t sp1);
void ejay_ds_broke(const char *callee, uint16_t ds0, uint16_t ds1);
...
static inline int cpu_lar(CPU *cpu, uint16_t sel, uint16_t *out) {
    if (sel == 0 || cpu->sel_base[sel] == EJAY_GUARD_BASE) return 0;
    *out = 0x9300;
    return 1;
}
...
/* Write watchpoint: set EJAY_WATCH=seg:off (hex) to get a host backtrace at
...
#ifdef EJAY_WATCH_MEM
extern uint16_t g_watch_seg, g_watch_off;
extern int g_watch_armed;
void ejay_watch_hit(uint16_t seg, uint16_t off, uint16_t val);
...
void ejay_watch_frame(uint16_t seg, uint16_t off, int on);
#define EJAY_WATCH_CHECK(s, o, v) \
    do { if ((s) == g_watch_seg && (o) == g_watch_off) ejay_watch_hit((s), (o), (v)); } while (0)
#else
#define EJAY_WATCH_CHECK(s, o, v) ((void)0)
#define ejay_watch_frame(s, o, n) ((void)0)
#endif
...
#define EJAY_SELW(s, o, v) do { if (g_wsel == 0xFFFFu) ejay_sel_write((s), 0, (v)); else if (g_wsel && ((s) == g_wsel || (s) == (uint16_t)(g_wsel + 1))) ejay_sel_write((s), (o), (v)); } while (0)

static inline void mem_write8(CPU *cpu, uint16_t seg, uint16_t off, uint8_t val) {
    EJAY_WATCH_CHECK(seg, off, val);
    EJAY_SELW(seg, off, val);
...
static inline void mem_write16(CPU *cpu, uint16_t seg, uint16_t off, uint16_t val) {
    ...
    if (seg == 0x42 && (off == 0x14 || (off >= 0xFFBC && off <= 0xFFC0))) {
        ...
        for (int _i = 16; _i > 0; _i--) {
            const char *nm = g_fn_ring[(g_fn_ring_pos - (unsigned)_i) & (EJAY_FN_RING_SIZE - 1)];
...
/* ADC/SBB. The carry is a THIRD input, and folding it into the source operand
 * loses it: flags_add16(a, b + cf) with b = 0FFFFh and CF set adds zero and
 * reports no carry out, so the high word of every long addition that lands on
 * all-ones comes out quietly wrong. One function per operation rather than one
 * per width; `bits` picks the masks. Taken from pcrecomp's
 * runtime/recomp16/cpu.h, which catz's fork of this header dropped.
 * DANCE02 needs both: its mixer runs 32-bit accumulators through dx:ax. */
...
static inline int cpu_alloc_mem(CPU *cpu, uint32_t size) {
    ...
    for (uint32_t s = 0; s < 65536u; s++)
        cpu->sel_base[s] = EJAY_GUARD_BASE;
    for (int s = 0; s <= EJAY_NUM_SEG; s++)
    ...
    cpu->heap_next = (EJAY_IMAGE_SIZE + 0xFu) & ~0xFu;
    cpu->heap_end = size;
    cpu->next_sel = 0x4000;
    return 1;
}
